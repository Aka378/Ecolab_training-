...
#def size(self):
    ...
    def bubble_sort(self):
        if self._first is None or self._first._next is None:
            return self
        ...
        while swapped:
            ...
            while current._next:
                if current._value > current._next._value:
                    current._value, current._next._value = current._next._value, current._value
                    swapped = True
                current = current._next
        
        return self
                
l = LinkedList(-1, 3, 9, 2, 5, 1, 1, 0, 4, 18)
sorted_list = l.bubble_sort()

# Print the sorted list
current_node = sorted_list._first
while current_node:
